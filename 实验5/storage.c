...
#define  clear_period  50           //清0周期
    ...
    //1.定义用户进程虚页控制结构 method1.空页面头指针 3. 忙页面头指针  4. 忙页面尾指针
    pfc_type pfc[total_vp], *freepf_head, *busypf_head, *busypf_tail;
    //1.页面失效次数  method1.a[]  指令序列
    int diseffect,  a[total_instruction];
    //1. 每条指令所在的页号 method1.每页装入10条指令后取模运算页号偏移值
    ...
    int main( )
    {
        ...
        for(i=0;i<total_instruction;i+=4)        /*产生指令队列*/
            ...
            if((a[i+2]> 318)|| (s>319))
                printf("a[%d+method1],a number which is :%d and s==%d\n",i,a[i+2],s);
            }
        //将指令序列转换成页号，偏移值
